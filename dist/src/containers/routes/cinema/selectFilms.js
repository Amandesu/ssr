"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = void 0;

var _react = _interopRequireWildcard(require("react"));

var _reactRedux = require("react-redux");

var _fetchData = _interopRequireDefault(require("../../../utils/fetchData"));

var _cinema = require("../../reduces/cinema");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _getRequireWildcardCache() { if (typeof WeakMap !== "function") return null; var cache = new WeakMap(); _getRequireWildcardCache = function () { return cache; }; return cache; }

function _interopRequireWildcard(obj) { if (obj && obj.__esModule) { return obj; } var cache = _getRequireWildcardCache(); if (cache && cache.has(obj)) { return cache.get(obj); } var newObj = {}; if (obj != null) { var hasPropertyDescriptor = Object.defineProperty && Object.getOwnPropertyDescriptor; for (var key in obj) { if (Object.prototype.hasOwnProperty.call(obj, key)) { var desc = hasPropertyDescriptor ? Object.getOwnPropertyDescriptor(obj, key) : null; if (desc && (desc.get || desc.set)) { Object.defineProperty(newObj, key, desc); } else { newObj[key] = obj[key]; } } } } newObj.default = obj; if (cache) { cache.set(obj, newObj); } return newObj; }

class SelectFilms extends _react.Component {
  constructor(props) {
    super(props);
    this.getFilmsShowTime = this.getFilmsShowTime.bind(this);
  }

  componentWillUpdate() {
    console.log(123);
  }

  renderServices(services) {
    console.log(services);
    return services && services.map((service, index) => {
      return _react.default.createElement("div", {
        key: index,
        style: {
          height: '18px',
          padding: '1px 3px',
          border: '1px solid #ffb232',
          marginRight: '12px'
        }
      }, service.name);
    });
  }

  static getInitialProps(props, cinemaId) {
    (0, _fetchData.default)({
      method: "GET",
      url: "mall.film-ticket.cinema.info",
      data: {
        cinemaId
      }
    }).then(res => {
      //console.log(res.data.cinema)
      props.changeCinema(res.data.cinema);
    });
  }

  getFilms(cinemaId, props) {
    return (0, _fetchData.default)({
      method: "GET",
      url: "mall.film-ticket.film.cinema-show-film",
      data: {
        cinemaId
      }
    }).then(res => {
      //console.log(res.data.films)
      props.getSetFilms(res.data.films);
    });
  }

  componentDidMount() {
    SelectFilms.getInitialProps(this.props, this.props.cinemaStore.cinema.cinemaId);
    this.getFilms(this.props.cinemaStore.cinema.cinemaId, this.props).then(() => {
      this.getFilmsShowTime(this.props.cinemaStore.films[0], this.props.cinemaStore.cinema.cinemaId, this.props, 0);
    }); //console.log(this.props.cinemaStore.films[0],this.props.cinemaStore.cinema.cinemaId,this.props)
    //this.getFilmsShowTime(this.props.cinemaStore.films[0],this.props.cinemaStore.cinema.cinemaId,this.props);
  }

  getFilmsShowTime(film, cinemaId, props, i) {
    (0, _fetchData.default)({
      method: "GET",
      url: "mall.film-ticket.schedule.list",
      data: {
        filmId: film.filmId,
        cinemaId,
        date: film.showDate[i],
        k: 3316308
      }
    }).then(res => {
      //console.log(res.data.schedules);
      props.getSchedules(res.data.schedules);
    });
  }

  setFilm(num, e) {
    //console.log(this.refs.films.children)
    //this.refs.films.children[0].style.padding = '0';
    const {
      films
    } = this.props.cinemaStore;
    const children = [...e.target.parentNode.children];
    children.forEach(child => {
      child.style.padding = '26px 9px 0';
    });
    e.target.style.padding = '0';
    e.target.parentNode.style.left = `${document.body.clientWidth / 2 - 45 - num * 120}px`; //console.log(this.refs.filmInfo.children[0]);

    let aDivs = this.refs.filmInfo.children;
    aDivs[0].children[0].textContent = films[num].name;
    aDivs[0].children[1].textContent = `${films[num].grade}分`;
    aDivs[1].textContent = films[num].actors ? `
        ${films[num].category}|${films[num].runtime}分钟|${films[num].director}|${films[num].actors.map(actor => actor.name).join(' ')}
        ` : `
        ${films[num].category}|${films[num].runtime}分钟|${films[num].director}
        `;
    /* 改变电影日期 */

    let filmDates = films[num].showDate;
    console.log(filmDates);
    this.props.setFilmDates(filmDates); //return this.renderDateInfo(filmDates);

    /* 改变放映表 */

    this.getFilmsShowTime(films[num], this.props.cinemaStore.cinema.cinemaId, this.props, 0);
    this.props.setNum(num);
  } //转换10位或者13位时间戳


  getDate(param) {
    if (param.toString().length == 10) {
      return new Date(param * 1000);
    } else if (param.toString().length == 13) {
      return new Date(param);
    }
  }

  renderDateInfo(_filmDates) {
    const {
      films,
      num
    } = this.props.cinemaStore;
    /* if(this._filmDate && this._filmDate.children[0]) {
        this._filmDate.children[0].style.color = '#ff5f16';
    } */

    return _react.default.createElement("div", {
      ref: div => this._filmDate = div,
      style: {
        height: '49px',
        fontSize: '14px',
        display: 'flex',
        alignItems: 'center'
      }
    }, _filmDates.map((filmDate, index) => {
      return _react.default.createElement("div", {
        onClick: e => {
          this.getFilmsShowTime(films[num], this.props.cinemaStore.cinema.cinemaId, this.props, index);
          Array.from(e.target.parentNode.children).forEach(item => item.style.color = '');
          e.target.style.color = '#ff5f16';
        },
        key: index,
        style: {
          padding: '0 15px'
        }
      }, `${filmDate.getMonth() + 1}月${filmDate.getDate()}日`);
    }));
  }

  renderSchedule() {
    const {
      schedules
    } = this.props.cinemaStore;
    return schedules.map(item => {
      const showAt = this.getDate(item.showAt);
      const endAt = this.getDate(item.endAt);

      const addZero = time => {
        if (time < 10) {
          return '0' + time;
        } else {
          return time;
        }
      };

      const priceColor = item.isOnsell ? '#ff5f16' : '#191a1b';
      return _react.default.createElement("div", {
        style: {
          height: '74px',
          padding: '15px',
          boxSizing: 'border-box',
          display: 'flex'
        }
      }, _react.default.createElement("div", {
        style: {
          width: '100px'
        }
      }, _react.default.createElement("div", {
        style: {
          height: '22px',
          fontSize: '15px'
        }
      }, addZero(showAt.getHours()), ":", addZero(showAt.getMinutes())), _react.default.createElement("div", {
        style: {
          height: '22px',
          fontSize: '13px',
          color: '#797d82'
        }
      }, addZero(endAt.getHours()), ":", addZero(endAt.getMinutes()), "\u6563\u573A")), _react.default.createElement("div", {
        style: {
          flex: 1
        }
      }, _react.default.createElement("div", {
        style: {
          height: '22px',
          fontSize: '15px'
        }
      }, item.filmLanguage, item.imagery), _react.default.createElement("div", {
        style: {
          height: '22px',
          fontSize: '13px',
          color: '#797d82'
        }
      }, item.hallName)), _react.default.createElement("div", {
        style: {
          width: '100px',
          padding: '10px 0',
          boxSizing: 'border-box',
          display: 'flex'
        }
      }, _react.default.createElement("div", {
        style: {
          width: '50px',
          paddingRight: '20px',
          fontSize: '13px',
          color: priceColor,
          lineHeight: '24px',
          boxSizing: 'border-box'
        }
      }, "\uFFE5", item.salePrice / 100), _react.default.createElement("div", {
        style: {
          width: '50px',
          border: `1px solid ${priceColor}`,
          color: priceColor,
          lineHeight: '24px',
          textAlign: 'center',
          fontSize: '12px'
        }
      }, item.isOnsell ? '购票' : '停售')));
    });
  }

  render() {
    const {
      cinema,
      schedules,
      films,
      filmDates
    } = this.props.cinemaStore;
    console.log(films);

    const _filmDates = filmDates.map(item => this.getDate(item));

    console.log(_filmDates);
    return _react.default.createElement("div", {
      style: {
        width: '100%',
        background: '#fff',
        color: '#2c3e50'
      }
    }, _react.default.createElement("div", {
      style: {
        height: '44px'
      }
    }, _react.default.createElement("img", {
      onClick: () => {
        this.props.history.push('/cinema');
      },
      style: {
        width: '11px',
        height: '18px',
        margin: '13px 0 0 15px'
      },
      src: "data:image/png;base64,"
    })), _react.default.createElement("div", {
      style: {
        height: '44px',
        lineHeight: '44px',
        textAlign: 'center',
        padding: '0 15px',
        fontSize: '17px',
        color: '#191a1b'
      }
    }, cinema.name), _react.default.createElement("div", {
      style: {
        height: '38px',
        padding: '5px 0 15px',
        boxSizing: 'border-box',
        color: '#ffb232',
        fontSize: '10px',
        display: 'flex',
        justifyContent: 'center'
      }
    }, this.renderServices(cinema.services)), _react.default.createElement("div", {
      style: {
        height: '50px',
        padding: '0 20px',
        boxSizing: 'border-box'
      }
    }, _react.default.createElement("div", {
      style: {
        height: '20px',
        margin: '15px 0',
        boxSizing: 'border-box',
        fontSize: '14px',
        display: 'flex',
        alignItems: 'center'
      }
    }, _react.default.createElement("img", {
      style: {
        width: '14px',
        height: '21px'
      },
      src: "data:image/png;base64,"
    }), _react.default.createElement("div", {
      style: {
        width: '100%',
        padding: '0 12px'
      }
    }, cinema.address), _react.default.createElement("img", {
      style: {
        width: '17px',
        height: '18px'
      },
      src: "data:image/png;base64,"
    }))), _react.default.createElement("div", {
      style: {
        width: '100%',
        height: '160px',
        padding: '15px 0',
        background: 'pink',
        boxSizing: 'border-box',
        position: 'relative'
      }
    }, _react.default.createElement("div", {
      ref: "films",
      style: {
        position: 'absolute',
        left: `${document.body.clientWidth / 2 - 45}px`,
        display: 'flex'
      }
    }, films.map((film, index) => {
      if (index != 0) {
        return _react.default.createElement("img", {
          onClick: this.setFilm.bind(this, index),
          style: {
            width: '90px',
            height: '130px',
            padding: '26px 9px 0',
            boxSizing: 'border-box',
            marginRight: '30px'
          },
          src: film.poster
        });
      } else {
        return _react.default.createElement("img", {
          onClick: this.setFilm.bind(this, index),
          style: {
            width: '90px',
            height: '130px',
            padding: '0',
            boxSizing: 'border-box',
            marginRight: '30px'
          },
          src: film.poster
        });
      }
    }))), _react.default.createElement("div", {
      ref: "filmInfo",
      style: {
        height: '80px',
        padding: '15px 0',
        boxSizing: 'border-box',
        fontSize: '12px'
      }
    }, _react.default.createElement("div", {
      style: {
        marginBottom: '10px',
        height: '19px',
        boxSizing: 'border-box',
        fontSize: '15px',
        display: 'flex',
        justifyContent: 'center'
      }
    }, _react.default.createElement("div", {
      style: {
        paddingRight: '10px'
      }
    }, films[0] && films[0].name), _react.default.createElement("div", {
      style: {
        color: '#ffb232'
      }
    }, films[0] && films[0].grade, "\u5206")), _react.default.createElement("div", {
      style: {
        height: '18px',
        padding: '0 74px',
        font: '13px',
        color: '#797d82',
        textAlign: 'center'
      }
    }, films[0] && `
                            ${films[0].category}|${films[0].runtime}分钟|${films[0].director}|${films[0].actors.map(actor => actor.name).join(' ')}
                        `)), this.renderDateInfo(_filmDates), this.renderSchedule(schedules));
  }

}

var _default = (0, _reactRedux.connect)(state => ({
  cinemaStore: state.cinema
}), dispatch => ({
  changeCinema: cinema => dispatch({
    type: _cinema.CHANGE_CINEMA,
    cinema
  }),
  getSchedules: schedules => dispatch({
    type: _cinema.GET_SCHEDULES,
    schedules
  }),
  getSetFilms: films => dispatch({
    type: _cinema.GET_SET_FILMS,
    films
  }),
  setFilmDates: filmDates => dispatch({
    type: _cinema.SET_FILM_DATES,
    filmDates
  }),
  setNum: num => dispatch({
    type: _cinema.SET_NUM,
    num
  })
}))(SelectFilms);

exports.default = _default;